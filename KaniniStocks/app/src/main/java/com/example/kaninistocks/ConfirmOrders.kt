...
import com.example.kaninistocks.databinding.ActivityConfirmOrdersBinding
...
class ConfirmOrders : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        create.setOnClickListener{
            if(count>0){
                CoroutineScope(Dispatchers.IO).launch {
                    ...
                    var result=apiClient.apiService.BuyStocks("Bearer "+token, Stocks(text,count))
                    withContext(Dispatchers.Main){
        ...
        myorders.setOnClickListener{
            var intent= Intent(this,MyStockActivity::class.java)
